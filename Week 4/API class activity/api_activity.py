...
def main():
    ...
    for review in reviews:
        ...
        top_rated_rows.append(
            {"app": review.get("app"), "rating": review.get("rating")}
        )

    top_rated_rows.sort(
        key=lambda row: row["rating"] if row["rating"] is not None else -1,
        reverse=True,
    )
    ...
    with open(TOP_RATED_CSV_OUTPUT, "w", newline="", encoding="utf-8") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=["app", "rating"])
        writer.writeheader()
